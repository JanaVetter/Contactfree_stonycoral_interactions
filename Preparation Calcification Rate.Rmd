...
Use Calcification Formula for G (calcification rate =  dif TA/2(umol/l) * incubation volumn(L) * water density (kg/L)/ Zeit(h) * surface corals(cm^2))
```{r}
# to calculate water density we need pressure, temperatur and salinity
TA_comp$pressure <- 10.186 # pressure is an average of the days
TA_comp$Temp <- 26 # temperature is an average of the days. Use temperature from the incubation not the titrator here (See Schneider and Erez 2006 (used seawater density from the seawater directly not the titrator))

# Sea waterdensity calculation with the package oce and the function swRho
# Salinity was measured every morning from the start bucket
TA_comp <- TA_comp %>% 
  mutate(Sea_water_density = oce::swRho(salinity = S, temperature = Temp, pressure = pressure)/1000) %>% 
...
#use formula 
TA_comp <- TA_comp %>%
  mutate(Calc_umol_h = (((TA_umolL_con - TA_umolL)/2) * 1.00036 * Sea_water_density) / time_L_h) # mean incubation volume (1.00036) is used
...
ggsave("Graphs/surfacearea_manuscript.png", width=17, height= 13, limitsize=FALSE, dpi= 700, arranged_surf)
```
